...
vid_folder = r'\videos\results_Qiwei_New_Iter3'

#RT3D
file_name_cam1 = r'\Crackle_20201203_00007DLC_resnet50_TestDec3shuffle1_1030000filtered'
file_name_cam2 = r'\Crackle_20201203_00008DLC_resnet50_TestDec3shuffle1_1030000filtered'
file_name_cam3 = r'\Crackle_20201203_00009DLC_resnet50_TestDec3shuffle1_1030000filtered'
file_name_cam4 = r'\Crackle_20201203_00010DLC_resnet50_TestDec3shuffle1_1030000filtered'
...
#RT3D
groundTruth_file_name = r'\Ground_truth_segments_2020-12-03-RT3D-2.txt'
...
dropout_size = 3 #frames in each segment
likelihood_lim = 0.9 #0~1, 1 being the most likely, 0 being the least likely
cams_list = [1,2,3,4] #which cams to use

markers_x = [1,4,7,10,13,16,19,22]
markers_y = [2,5,8,11,14,17,20,23]
markers_l = [3,6,9,12,15,18,21,24] #likelihood
markers_f = [0] #frames
...
overall_outliers = sum(wholeArray > 20)/wholeArray.shape[0]
...
cams_columns_names = ['cam1','cam2','cam3','cam4']
cams_count = 0
markers_rows_names = ['shoulder','elbow1','elbow2','wrist1','wrist2','hand1','hand2','hand3']
markers_rows_nums = [0,1,2,3,4,5,6,7]
...
for i in range(len(cam_lists_outlier_original_wholeSection)): #camreas, columns
    ...
    for j in range(len(markers_rows_nums)): #markers, rows
        ...
        """
        if i == 0:
            print(str(cams_count))
            if cams_count < len(cam_lists_outlier_original_wholeSection):    
                plt.title(cams_columns_names[cams_count],fontsize = 16)
                cams_count += 1
        if j%len(cam_lists_outlier_original_wholeSection) == 0:
            plt.ylabel(markers_rows_names[markers_count],fontsize = 16,labelpad = 30)
            markers_count += 1
        """
        if i == 0:
        ...
        if len(cam_lists_outlier_original_wholeSection[i][j]) == 0:
            ...
            x_val_orig = cam_lists_outlier_original_wholeSection[i][markers_rows_nums[j]][0][:,x_column:x_column+1]
            y_val_orig = cam_lists_outlier_original_wholeSection[i][markers_rows_nums[j]][0][:,y_column:y_column+1]
            
            x_val_interp = cam_lists_outliers_interpolated_wholeSection[i][markers_rows_nums[j]][0][:,x_column:x_column+1]
            y_val_interp = cam_lists_outliers_interpolated_wholeSection[i][markers_rows_nums[j]][0][:,y_column:y_column+1]
