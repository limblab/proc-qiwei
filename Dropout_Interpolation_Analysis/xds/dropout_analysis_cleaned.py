...
def dropout(file,percentage,dropout_list):
    ...
    #why da fuk I wrote this twice, but I fear too much to delete it
    ...
    for k in range(dropout_numbers_check.shape[0]):
        if dropout_numbers_check[k] != 0:
            while dropout_numbers_check[k] > 0:
                ...
                #Magic solution, to prevent any random numbers go less than 20000, so 
                #that... the further calculation (use the 1000 times of data points before the 
...
#%%
"""
Taking a look at how interpolation looks like, not related to the whole file
"""
...
1. call dropout(), do dropout on different levels (0.03,0.04...)
2. For each of the above, call rebin(), do rebin10，20，30，40，50
3. For each of the above, call smooth_binned_spikes()
4. For each, call separate(), separate training and testing, calculate VAF results
"""
"""
TODO: --init--.py, util.py, main.py
TODO: use scipy.io to save dropout matrix as a file
TODO: use binned data to do interpolation, take data in front of or in the back of the dropout site as well
"""
    ...
    for file in files:
        ...
        if filepath.endswith(".mat"):
            ...
            for a in range(len(dropout_percentage)):
                ...
                (in 0.001s), but the unit/resolution for dropout_probability_list
                is in 100Hz. dropout_timepoints_length[i]*10 will be 1ms in 
                spike_counts_dropout.
                """
                ...
                for b in range(len(bin_array)): #5
                    ...
                    #Why I didn't append vaf3 is because I didn't reserve a place
                    #for VAF after interpolation (since I know it will be bad)
                    #and I just want to see how the interpolation VAF looks like
...
"""
Starting from this block it's all about ... saving files and plots.
"""

with open('dropout_VAF_result.txt', 'a') as f:
    for item in VAF_list:
        f.write("%s\n" % item)
f.close()
 
#%% Open results for further analysis
f = open('dropout_VAF_result.txt','r')
contents = f.read()
f.close()
...
#%%
"""
Plot out and see how VAF values change when changing different variables
"""
