...
% end
%         
addpath('ClassyDataAnalysis\lib\NPMK')
addpath('proc-kevin\Helpful_Functions')
...
DropoutProb = zeros(1,1000);
for j = 1:length(histData)
    DropoutProb(fix(histData(j)*100)) = DropoutProb(fix(histData(j)*100)) + 1;
end
DropoutProb = DropoutProb/6342;
%%
csvwrite('test.txt',DropoutProb.')
