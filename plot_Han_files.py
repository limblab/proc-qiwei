...
def plot_behave_dict(behave_dict, plot_len, bin_size):
    ...
    for i in range(N):
        ax0 = plt.subplot(grid[i+spike_grid,0])
        p1 = p_cont[plot_start:plot_start+plot_len]
        #plt.yticks([])
        frame = plt.gca()
...
"""
path = 'E:/Box Sync/Lab updates/Qiwei/'
...
#%%
# behave1 = [1,	20] # Provide the number of frames here
...
behave_dict['cont'] = np.ones((len(idx), 1))

fig = plt.figure('rasters', figsize=(11, 10))
plt.subplots_adjust(left=0.1, right=0.8, top=0.9, bottom=0.1)
plot_behave_dict(behave_dict, 0, 0.001)
